...
public class MuelleControllerTest {
    ...
    Muelle muelle1 = new Muelle(1, "A" );
    Muelle muelle2 = new Muelle(muelle1.getIdMuelle(), "B" );
    Muelle muelle3 = new Muelle(2, "C");
...
try{
    ...
    assertEquals(muelleController.buscar(muelle1.getIdMuelle()));
}catch(Exception ex){
...
try{
    ...
    assertTrue(assertTrue(ex.getMessage().contains("El muelle no existe"));
}
}
...
public void validarMuelleTest(){
    ...
    try{
        muelleController.validar(new Muelle(0,  null));
    }catch(Exception ex){
    ...
    try{
        muelleController.validar(new Muelle(0, "A"));
    }catch(Exception ex){
        assertTrue(ex.getMessage().contains("El id del muelle no puede ser cero o menor" ));
    }
}
